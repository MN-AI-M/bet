(() => {
    ...
    function legalMoves(board, player) {
        ...
        for (let r = 0; r < 9; r += 1) {
            for (let c = 0; c < 9; c += 1) {
                ...
                if (cell.owner === 0 && cell.promisedOwner !== player) moves.push({
                    r,
                    c
                })
            }
    ...
    function chooseTacticalMove() {
        ...
        if (opponentWinning.length) {
            const forcedBlocks = legal.filter((move) => opponentWinning.some((threat) => threat.r === move.r && threat.c === move.c));
        ...
        for (const move of legal) {
            ...
            const lineScore = typeof evaluateCellImpact === 'function' ? evaluateCellImpact(board, move.r, move.c, BOT) : 0;
    ...
    window.getBotBestMoveTreeSearch = () => {
        ...
        return typeof originalSearch === 'function' ? originalSearch() : {
            r: 4,
            c: 4
        }
    }
