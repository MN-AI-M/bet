(() => {
  ...
  let boardCaptureInstalled = false;
  let skillDragInstalled = false;
  ...
  function ensureState() {
    ...
    state.lineCounts = state.lineCounts || { 1: 0, 2: 0 };
    state.scoredLines = state.scoredLines || { 1: {}, 2: {} };
    state.scoreGameOver = false;
  }
  ...
  function updateScoreUI() {
    ...
    const lines = state.lineCounts || { 1: 0, 2: 0 };
    [1, 2].forEach((player) => {
  ...
  function finishScoreGame(winner) {
    ...
    state.scoreGameOver = true;
    state.isGameOver = true;
    const winnerName = winner === state.myOnlinePlayer && typeof window.skillTacPlayerName === 'function'
      ? window.skillTacPlayerName()
      : winner === 2 && state.mode === 'pve' ? 'Bot' : `Player ${winner}`;
  ...
  function sendScoreMove(move) {
    if (state.mode === 'pvp_online' && conn && state.currentPlayer === state.myOnlinePlayer) {
      conn.send({ type: 'SCORE_PLACE', player: state.myOnlinePlayer, r: move.r, c: move.c, scores: state.scores, lineCounts: state.lineCounts });
    }
  }

  function applyScoreMove(move, remote = false) {
  ...
  function installScoreSkillDrag() {
    ...
    elements.skillsList.addEventListener('pointermove', updateDragVisuals, true);
  ...
  function setScoreBot() {
    ...
    state.scores = { 1: 0, 2: Math.floor(Math.random() * 501) };
    state.lineCounts = { 1: 0, 2: 0 };
    startGame();
    installBoardCapture();
