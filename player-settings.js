(() => {
  ...
  if (tutorialOverlay) {
    ...
    }).observe(tutorialOverlay, { attributes: true, attributeFilter: ['class'] });
  }

  const status = document.getElementById('status-turn');
  if (status) new MutationObserver(refreshNames).observe(status, { childList: true, characterData: true, subtree: true });
  ...
  window.handleP2PData = (message) => {
    if (message && message.type === 'HELLO' && typeof conn !== 'undefined' && conn) {
      conn.send({ type: 'PLAYER_NAME', name });
    }
