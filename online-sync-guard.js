(() => {
    ...
    window.handleCellClick = function(row, col) {
        if (state.mode === 'matching' || state.mode === 'menu' || isAiThinking || state.isGameOver || (state.mode === 'pve' && state.currentPlayer === 2) || !canUseOnlineTurn()) return;
    ...
    window.handleP2PData = function(message) {
        ...
        try {
            ...
            if (message.type === 'USE_SKILL') executeSkill(message.skillId, message.target ? {
                dataset: {
                    row: message.target.r,
                    col: message.target.c
                }
            } : null)
        } finally {
